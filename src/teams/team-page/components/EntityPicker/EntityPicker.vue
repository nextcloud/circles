...
<template>
	<Modal
		...
		<div class="entity-picker">
			...
			<div class="entity-picker__search-container">
				<div class="entity-picker__search">
					...
					<input
						ref="input"
						v-model="searchQuery"
						:placeholder="t('circles', 'Search {types}', { types: searchPlaceholderTypes })"
				...
				<NcButton
					...
					:title="t('circles', 'Add guest')"
					:aria-label="t('circles', 'Add guest')"
			...
			<!-- Loading -->
			<EmptyContent v-if="loading" :name="t('circles', 'Loading …')">
				<template #icon>
					<IconLoading :size="20" />
				</template>
			</EmptyContent>
			...
			<template v-else>
				...
				<EmptyContent v-else-if="searchQuery" :name="t('circles', 'No results')">
					<template #icon>
						<IconSearch :size="20" />
					</template>
				</EmptyContent>
