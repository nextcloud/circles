...
use OCA\Files_Sharing\Cache;
use OCA\Files_Sharing\External\Scanner;
use OCA\Files_Sharing\External\Watcher;
...
class RemoteStorage extends DAV implements ISharedStorage {
	...
	//	$this->httpClient = $options['HttpClientService'];
		...
		$this->remoteMount = $options['remote_mount'];
		$discoveryService = \OC::$server->query(IDiscoveryService::class);

		list($protocol, $remote) = explode('://', $this->remoteMount->getRemoteCloud()->getAddress());
		if (strpos($remote, '/')) {
		...
		$federatedSharingEndpoints =
			$discoveryService->discover($this->remoteMount->getRemoteCloud()->getAddress(), 'FEDERATED_SHARING');
		$webDavEndpoint =
			isset($federatedSharingEndpoints['webdav']) ? $federatedSharingEndpoints['webdav'] : '/public.php/webdav';
		...
		parent::__construct(
			array(
				...
				'host'     => $host,
				'root'     => $root,
				'user'     => $this->remoteMount->getToken(),
	...
	public function getRemoteUser() {
		return $this->remoteMount->getAuthor();
	}

	public function getRemote() {
		return $this->remoteMount->getRemoteCloud()->getAddress();
	}

	public function getMountPoint() {
		return $this->remoteMount->getMountPoint();
	}

	public function getToken() {
		return $this->remoteMount->getToken();
	}

	public function getPassword() {
		return $this->remoteMount->getPassword();
	}
	...
	public function getId() {
		return 'shared::' . md5($this->getToken() . '@' . $this->getRemote());
	}

	public function getCache($path = '', $storage = null) {
		if (is_null($this->cache)) {
			$this->cache = new Cache($this, $this->remoteMount->getRemoteCloud()->getAddress());
		}
	...
	public function checkStorageAvailability() {
		...
		try {
			...
			if ($this->testRemote()) {
				...
				$this->manager->getMountManager()
							  ->removeMount($this->mountPoint);
	...
	public function getShareInfo() {
		...
		try {
			...
			if ($e->getCode() === Http::STATUS_UNAUTHORIZED
				|| $e->getCode() === Http::STATUS_FORBIDDEN) {
