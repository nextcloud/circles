...
class NodeEventListener implements IEventListener {
	...
	public function handle(Event $event): void {
		...
		try {
			...
			$this->logger->debug('Failed to process node event: ' . $e->getMessage());
		}
	}
	...
	private function invalidateCacheForNode(Node $node): void {
		...
		try {
			...
			$this->logger->debug('Failed to get shares for node ' . $node->getId() . ': ' . $e->getMessage());
		}
		...
		try {
			...
			while (true) {
				...
				if (isset($visitedNodeIds[$parent->getId()])) {
					$this->logger->debug(
						'Detected cycle in folder hierarchy at node ' . $parent->getId()
					);
					break;
				}
