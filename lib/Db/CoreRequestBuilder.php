...
class CoreRequestBuilder extends NC22ExtendedQueryBuilder {
	...
	private function leftJoinMemberFromInstance(
		...
		$expr = $this->expr();
		$this->leftJoin(
			$this->getDefaultSelectAlias(), CoreQueryBuilder::TABLE_MEMBER, $aliasRemoteMember,
			$expr->andX(
	...
	protected function limitRemoteVisibility(string $alias, bool $sensitive, string $aliasCircle) {
		...
		if (!$sensitive) {
			...
			if ($this->getDefaultSelectAlias() === CoreRequestBuilder::MEMBER) {
...
//			);
	...
	public function leftJoinOwner(string $alias, string $field = 'unique_id'): void {
		...
		$this->generateMemberSelectAlias($aliasMember)
			 ->leftJoin(
				 $alias, CoreQueryBuilder::TABLE_MEMBER, $aliasMember,
				 $expr->andX(
...
//		}
	...
	public function limitToInheritedMemberships(string $alias, string $singleId, string $field = ''): void {
		...
		$this->leftJoin(
			$alias, CoreQueryBuilder::TABLE_MEMBERSHIP, $aliasUpstreamMembership,
...
//			)
		...
		$this->generateMemberSelectAlias($aliasInheritanceFrom)
			 ->leftJoin(
				 $aliasMembership, CoreQueryBuilder::TABLE_MEMBER, $aliasInheritanceFrom,
				 $expr->andX(
...
//		$qb->andWhere($orXTypes);
	...
	public function leftJoinShareChild(string $aliasShare, string $aliasShareMemberships = '') {
		...
		$this->leftJoin(
			$aliasShareMemberships, CoreQueryBuilder::TABLE_SHARE, $aliasShareChild,
			$expr->andX(
...
//		$this->selectAlias($aliasShareParent . '.permissions', 'parent_perms');
	...
	public function leftJoinMountpoint(string $aliasMount, string $aliasMountMemberships = '') {
		...
		$this->leftJoin(
			$aliasMountMemberships, CoreQueryBuilder::TABLE_MOUNTPOINT, $aliasMountpoint,
			$expr->andX(
