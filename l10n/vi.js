OC.L10N.register(
    "circles",
    {
    "Personal circle" : "Vòng cá nhân",
    "Hidden circle" : "Vòng ẩn",
    "Private circle" : "Vòng riêng tư",
    "Public circle" : "Vòng công cộng",
    "Circle" : "Vòng kết nối",
    "Personal Circle" : "Vòng cá nhân",
    "Hidden Circle" : "Vòng ẩn",
    "Private Circle" : "Vòng riêng tư",
    "Public Circle" : "Vòng công cộng",
    "Personal" : "Cá nhân",
    "Hidden" : "Ẩn",
    "Private" : "Riêng tư",
    "Public" : "Công cộng",
    "Not a member" : "Không phải là thành viên",
    "Member" : "Thành viên",
    "Moderator" : "Kiểm duyệt",
    "Admin" : "Quản trị",
    "Owner" : "Chủ",
    "Unknown" : "Không xác định",
    "Invited" : "Được mời",
    "Requesting" : "Yêu cầu",
    "Blocked" : "Bị chặn",
    "Kicked" : "Bị đuổi",
    "Are you sure you want to delete this circle?" : "Bạn có chắc chắn muốn xóa vòng kết nối này?",
    "Kick this member" : "Đuổi thành viên này",
    "Accept the request" : "Chấp nhận yêu cầu",
    "Dismiss the request" : "Loại bỏ yêu cầu",
    "Cannot join this circle" : "Không thể tham gia vòng kết nối này",
    "no error message" : "không có thông báo lỗi",
    "You have requested to join this circle" : "Bạn đã yêu cầu tham gia vòng kết nối này",
    "Cannot leave this circle" : "Không thể rời khỏi vòng kết nối này",
    "Cannot delete this circle" : "Không thể xóa vòng kết nối này",
    " {type} '{name}' created" : "{nhập} '{tên}' đã tạo",
    " {type} '{name}' could not be created" : "{nhập} '{tên}' không thể tạo",
    "Issue while retrieving the details of this circle" : "Lỗi khi truy xuất chi tiết của vòng kết nối này",
    "Issue while retrieving the list of circles" : "Lỗi trong khi truy xuất danh sách vòng kết nối",
    "Member '{name}' could not be added to the circle" : "Không thể thêm thành viên '{name}' vào vòng kết nối",
    "Member '{name}' could not be removed from the circle" : "Không thể xóa thành viên '{name}' khỏi vòng kết nối",
    "Circles" : "Vòng kết nối",
    "The name of your circle cannot start with this character" : "Tên vòng kết nối của bạn không thể bắt đầu bằng ký tự này",
    "You cannot use the Circles Application until your administrator has allowed at least one type of circles" : "Bạn không thể sử dụng ứng dụng Vòng kết nối cho đến khi quản trị viên cho phép ít nhất một loại vòng kết nối",
    "The circle does not exist or is hidden" : "Vòng kết nối không tồn tại hoặc ẩn",
    "A circle with that name exists" : "Đã tồn tại một vòng kết nối có tên đó",
    "This member does not exist" : "Thành viên này không tồn tại",
    "This user is already a member of the circle" : "Người dùng này đã là thành viên của vòng kết nối",
    "You cannot join this circle" : "Bạn không thể tham gia vòng kết nối này",
    "This member is not a moderator" : "Thành viên này không phải là người kiểm duyệt",
    "This member is not the owner of the circle" : "Thành viên này không phải là chủ của vòng kết nối",
    "This member is the owner of the circle" : "Thành viên này là chủ của vòng kết nối",
    "You are already a member of this circle" : "Bạn đã là thành viên của vòng kết nối này",
    "You have been blocked from this circle" : "Bạn đã bị chặn khỏi vòng kết nối này",
    "You need a specify a type of circle" : "Bạn cần xác định một loại vòng kết",
    "You cannot create this type of circle" : "Bạn không thể tạo loại vòng kết nối này",
    "You cannot display this type of circle" : "Bạn không thể hiển thị loại vòng kết nối này",
    "This user does not exist" : "Người dùng này không tòn tại",
    "Create a new circle" : "Tạo một vòng kết nối mới",
    "Select a type of circle" : "Chọn loại vòng kết nối",
    "Create a personal circle" : "Tạo vòng kết nối cá nhân",
    "Create an hidden circle" : "Tạo vòng kết nối ẩn",
    "Create a private circle" : "Tạo vòng kết nối riêng tư",
    "Create a public circle" : "Tạo vòng kết nối công cộng",
    "A personal circle is a list of users known only to the owner." : "Vòng kết nối cá nhân là danh sách những người dùng mà chỉ chủ sở hữu biết.",
    "This is the right option if you want to do recurrent sharing with the same group." : "Đây là lựa chọn đúng nếu bạn muốn chia sẻ định kỳ với cùng một nhóm.",
    "An hidden circle is an open group that can be protected by a password." : "Một vòng kết nối ẩn là một nhóm mở có thể được bảo vệ bằng mật khẩu.",
    "Users won't be able to find this circle using the Nextcloud search engine." : "Người dùng sẽ không thể tìm thấy vòng kết nối này bằng công cụ tìm kiếm Nextcloud.",
    "A private circle requires invitation or confirmation by an admin." : "Một vòng kết nối riêng tư yêu cầu lời mời hoặc xác nhận của quản trị viên.",
    "This is the right circle if you are looking for privacy when sharing your files or ideas." : "Đây là lựa chọn vòng kết nối đúng nếu bạn đang tìm kiếm sự riêng tư khi chia sẻ tệp hoặc ý tưởng của mình.",
    "A public circle is an open group visible to anyone willing to join." : "Một vòng kết nối công cộng là một nhóm mở được hiển thị cho bất kỳ ai muốn tham gia.",
    "Everyone will be able to see and join your circle." : "Mọi người có thể thấy và tham gia vòng kết nối của bạn.",
    "personal circles" : "vòng kết nối cá nhân",
    "hidden circles" : "vòng kết nối ẩn",
    "private circles" : "vòng kết nối riêng",
    "public circles" : "vòng kết nối công cộng",
    "All circles" : "Tất cả vòng kết nối",
    "Search circles" : "Tìm kiếm vòng kết nối",
    "No filter" : "Không có bộ lọc",
    "Circles you belong to" : "Vòng kết nối bạn tham gia",
    "Circles you own" : "Vòng kết nối của bạn",
    "No circle selected" : "Không chọn vòng kết nối",
    "Pending invitation to join this circle" : "Lời mời tham gia vòng kết nối này",
    "Accept the invitation" : "Chấp nhận lời mời",
    "Decline the invitation" : "Từ chối lời mời",
    "Join this circle" : "Tham gia vòng kết nối này",
    "Leave this circle" : "Rời khỏi vòng kết nối này",
    "Add a member" : "Thêm thành viên",
    "Username" : "Tên người dùng",
    "Level" : "Cấp độ",
    "Status" : "Trạng thái",
    "Joined" : "Đã tham gia"
},
"nplurals=1; plural=0;");
