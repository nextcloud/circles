OC.L10N.register(
    "circles",
    {
    "You created the team {circle}" : "Tu izveidoji komandu {circle}",
    "You deleted {circle}" : "Tu izdzēsi {circle}",
    "You added team {member} as member to {circle}" : "Tu pievienoji komandu {member} kā {circle} dalībnieku",
    "You removed {member} from {circle}" : "Tu noņēmi {member} no {circle}",
    "You added {member} as member to {circle}" : "Tu pievienoji {member} kā {circle} dalībnieku",
    "You have been added as member to {circle} by {author}" : "{author} Tevi pievienoja kā {circle} dalībnieku",
    "You added {external} to {circle}" : "Tu {circle} pievienoji {external}",
    "You left {circle}" : "Tu pameti {circle}",
    "{member} left {circle}" : "{member} pameta {circle}",
    "You removed {external} from {circle}" : "Tu noņēmi {external} no {circle}",
    "You changed {member}'s level in {circle} to %1$s" : "Tu nomainīji {member} līmeni {circle} uz %1$s",
    "{author} changed your level in {circle} to %1$s" : "{author} nomainīja Tavu līmeni {circle} uz %1$s",
    "Teams" : "Komandas",
    "Personal" : "Personīgs",
    "System" : "Sistēma",
    "Visible" : "Redzams",
    "Open" : "Atvērt",
    "Invite" : "Uzaicināt",
    "Password protected" : "Aizsargāts ar paroli",
    "No Owner" : "Nav īpašnieka",
    "Hidden" : "Paslēpts",
    "Local" : "Vietējs",
    "Root" : "Root",
    "Federated" : "Federated",
    "App" : "Lietotne",
    "Email address" : "E-pasta adrese",
    "Contact" : "Kontaktpersona",
    "Team" : "Komanda",
    "Moderator" : "Satura pārraudzītājs",
    "Admin" : "Pārvaldītājs",
    "Owner" : "Īpašnieks",
    "Open »%s«" : "Atvērt »%s«",
    "Show all teams" : "Rādīt visas komandas",
    "Files" : "Datnes",
    "Accept" : "Akceptēt",
    "Invalid file provided" : "Norādīta nederīga datne",
    "File is too big" : "Datne ir pārāk liela.",
    "Invalid image" : "Nederīgs attēls",
    "Unknown filetype" : "Nezināms datnes veids",
    "%s" : "%s",
    "Open %s" : "Atvērt %s",
    "Unlimited" : "Neierobežota",
    "Cancel" : "Cancel",
    "Default quota" : "Apjoms pēc noklusējuma",
    "Save" : "Save",
    "Actions" : "Darbības",
    "Add" : "Pievienot",
    "Connect" : "Savienoties",
    "Members" : "Dalībnieki",
    "Add teams" : "Pievienot komandas",
    "Other" : "Cits",
    "Advanced" : "Paplašināti",
    "Name" : "Vārds",
    "Size" : "Izmērs",
    "Modified" : "Mainīts",
    "Home" : "Sākums",
    "Could not rename the page" : "Lapu nevarēja pārdēvēt",
    "description" : "apraksts",
    "Description" : "Apraksts",
    "Password protection" : "Paroles aizsardzība",
    "Apply" : "Apstiprināt",
    "Create link" : "Izveidot saiti",
    "Copy link" : "Ievietot saiti starpliktuvē",
    "Search {types}" : "Meklēt {types}",
    "No results" : "Nav iznākuma",
    "Pending" : "Gaida",
    "Role" : "Loma",
    "Add to {team}" : "Pievienot {team}",
    "No teams found" : "Netika atrasta neviena komanda",
    "Loading" : "Ielādē",
    "Go back" : "Atpakaļ",
    "Join" : "Pievienoties",
    "Group folders" : "Grupu mapes",
    "Loading…" : "Ielādē…",
    "User" : "Lietotājs",
    "Group" : "Grupa",
    "Email" : "E-pasts",
    "Federation" : "Federācija",
    "Privacy" : "Privātums",
    "Users" : "Lietotāji",
    "Groups" : "Grupas",
    "Contacts" : "Kontakti",
    "No files in here" : "Šeit nav datņu",
    "No entries found in this folder" : "Šajā mapē nekas nav atrasts"
},
"nplurals=3; plural=(n%10==1 && n%100!=11 ? 0 : n != 0 ? 1 : 2);");
