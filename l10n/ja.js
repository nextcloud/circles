OC.L10N.register(
    "circles",
    {
    "Are you sure you want to transfer your ownership?" : "本当にあなたの所有権を委譲しますか？",
    "Personal circle" : "パーソナルサークル",
    "Hidden circle" : "非公開サークル",
    "Private circle" : "プライベートサークル",
    "Public circle" : "公開サークル",
    "Circle" : "サークル",
    "Personal Circle" : "パーソナルサークル",
    "Hidden Circle" : "非公開サークル",
    "Private Circle" : "プライベートサークル",
    "Public Circle" : "公開サークル",
    "Personal" : "パーソナル",
    "Hidden" : "非公開",
    "Private" : "プライベート",
    "Public" : "公開",
    "Member" : "メンバー",
    "Admin" : "管理者",
    "Owner" : "オーナー",
    "Unknown" : "不明",
    "Cannot join this circle" : "このサークルに参加することはできません",
    "Cannot leave this circle" : "このサークルを抜けることはできません",
    "You have deleted this circle" : "あなたはこのサークルから除名されました",
    "Cannot delete this circle" : "このサークルを削除できません",
    "Circles" : "サークル",
    "You created the circle {circle}" : "あなたはサークル {circle} を作成しました",
    "{author} created the circle {circle}" : "{author} が {circle} を作成しました",
    "You deleted {circle}" : "あなたは {circle} を削除しました",
    "{author} deleted {circle}" : "{author} が {circle} を削除しました",
    "You joined {circle}" : "あなたは {circle} に参加しました",
    "{member} joined {circle}" : "{member} が {circle} に参加しました",
    "You added {member} as member to {circle}" : "あなたは {member} を {circle} にメンバーとして追加しました",
    "You were added as member to {circle} by {author}" : "あなたは {author} により {circle} にメンバーとして追加されました",
    "{member} was added as member to {circle} by {author}" : "{member} は {author} により {circle} にメンバーとして追加されました。",
    "You left {circle}" : "あなたは {circle} を抜けました",
    "{member} left {circle}" : "{member} が {circle} を抜けました",
    "You removed {member} from {circle}" : "あなたは {member} を {circle} から除名しました",
    "You were removed from {circle} by {author}" : "あなたは {author} により {circle} を除名されました",
    "{member} was removed from {circle} by {author}" : "{member} は {author} により {circle} を除名されました",
    "You invited {member} into {circle}" : "あなたは {member} を {circle} に招待しました",
    "You have been invited into {circle} by {author}" : "あなたは {author} より {circle} へ招待されています",
    "{member} have been invited into {circle} by {author}" : "{member} は {author} により {circle} に招待されています",
    "You are the new owner of {circle}" : "あなたは {circle} の新しいオーナーです",
    "{member} is the new owner of {circle}" : "{member} は {circle} の新しいオーナーです",
    "Circle not found" : "サークルが見つかりません",
    "This member does not exist" : "このメンバーは存在しません",
    "You cannot join this circle" : "あなたはこのサークルに加入できません",
    "Not enough privileges" : "十分な権限がありません",
    "Select a type of circle" : "サークルの種類を選択",
    "Create a personal circle" : "パーソナルサークルを作成",
    "Create a hidden circle" : "非公開サークルを作成",
    "Create a private circle" : "プライベートサークルを作成",
    "Create a public circle" : "公開サークルを作成",
    "A personal circle is a list of users known only to the owner." : "パーソナルサークルはオーナーが知っているユーザだけ表示されます。",
    "This is the right option if you want to do recurrent sharing with the same group." : "あなたが同じグループと頻繁に共有する場合に適切な選択肢です。",
    "An hidden circle is an open group that can be protected by a password." : "非公開サークルはパスワードで保護された公開グループです。",
    "Users won't be able to find this circle using the Nextcloud search engine." : "Nextcloud の検索エンジンでこのサークルを見つけることはできません。",
    "A private circle requires invitation or confirmation by an admin." : "プライベートサークルは招待か管理者による承認が必要です。",
    "This is the right circle if you are looking for privacy when sharing your files or ideas." : "あなたのファイルやアイデアを共有する際にプライバシーが必要な場合に適切なサークルです。",
    "A public circle is an open group visible to anyone willing to join." : "公開サークルは誰でも参加できるオープンなサークルです。",
    "Everyone will be able to see and join your circle." : "全ての人が見たり参加することが可能なサークルです。",
    "All circles" : "全てのサークル",
    "Search circles" : "サークルを検索",
    "No filter" : "フィルタ無し",
    "Circles you belong to" : "あなたが参加しているサークル",
    "Circles you own" : "あなたの所有するサークル",
    "No circle selected" : "サークルが選択されていません",
    "Add a member" : "メンバーの追加",
    "Join this circle" : "このサークルに参加",
    "Delete circle" : "サークルを削除",
    "Edit circle" : "サークルを編集",
    "Accept the invitation" : "招待を受ける",
    "Decline the invitation" : "招待を断る",
    "Leave this circle" : "このサークルを抜ける",
    "Return to menu" : "メニューに戻る",
    "Username" : "ユーザ名"
},
"nplurals=1; plural=0;");
