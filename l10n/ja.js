OC.L10N.register(
    ...
    "Unlimited" : "無制限",
    "Unable to update team space config" : "チームスペースの設定を更新できませんでした",
    "Changed default team space quota" : "チームスペースのデフォルトの容量制限を変更しました",
    "Quota must be a non-negative number." : "クォータは負でない数値にする必要があります",
    "Configure the default storage quota for team spaces. Requires the Team Folders app to be installed and enabled." : "チームスペースのデフォルトのストレージクォータを設定します。この設定を行うには、「Team Folders」アプリがインストールされ、有効化されている必要があります。",
    "Default quota" : "デフォルトのクオータ",
    "Select default quota" : "デフォルトのクオータを選択",
    "Save" : "保存する",
    "Default storage quota applied to each auto-created team space. Use 0 for unlimited storage." : "自動作成される各チームスペースに適用されるデフォルトのストレージクォータ。ストレージ容量を無制限にする場合は「0」を指定してください。",
