OC.L10N.register(
    "circles",
    {
    "Are you sure you want to transfer your Owner rights ?" : "Er du sikker på at du vil overføre dine eierskapsrettigheter?",
    "This action is irreversible" : "Denne handlingen kan ikke angres.",
    "Personal circle" : "Personlig sirkel",
    "Hidden circle" : "Skjult sirkel",
    "Private circle" : "Privat sirkel",
    "Public circle" : "Offentlig sirkel",
    "Circle" : "Sirkel",
    "Link Removed" : "Lenke fjernet",
    "Link down" : "Lenke nede",
    "Setting link" : "Setter lenke",
    "Request dismissed" : "Forespørsel avslått",
    "Request sent" : "Forespørsel sendt",
    "Link requested" : "Lenke forespurt",
    "Link up" : "Lenke oppe",
    "Personal Circle" : "Personlig sirkel",
    "Hidden Circle" : "Skjult sirkel",
    "Private Circle" : "Privat sirkel",
    "Public Circle" : "Offentlig sirkel",
    "Personal" : "Personlig",
    "Hidden" : "Skjult",
    "Private" : "Privat",
    "Public" : "Offentlig",
    "Not a member" : "Ikke et medlem",
    "Member" : "Medlem",
    "Moderator" : "Moderator",
    "Admin" : "Admin",
    "Owner" : "Eier",
    "Unknown" : "Ukjent",
    "Invited" : "Invitert",
    "Requesting" : "Forespør",
    "Blocked" : "Blokkert",
    "Kicked" : "Utkastet",
    "Are you sure you want to leave this circle?" : "Er du sikker på at du vil forlate denne sirkelen?",
    "Please confirm" : "Bekreft",
    "Are you sure you want to delete this circle?" : "Er du sikker på at du vil slette denne sirkelen?",
    "Kick this member" : "Kast ut dette medlemmet",
    "Accept the request" : "Aksepter forespørselen",
    "Dismiss the request" : "Avslå forespørselen",
    "Unlink this group" : "Fjern denne gruppen",
    "Remove this Link" : "Fjern denne lenken",
    "Cancel the Link request" : "Avbryt denne lenkeforespørselen",
    "Accept the Link request" : "Godta denne lenkeforespørselen",
    "Reject the Link request" : "Avslå lenkeforespørselen",
    "Cannot join this circle" : "Kan ikke ta del i denne sirkelen",
    "no error message" : "ingen feilmelding",
    "You have successfully joined this circle" : "Du har tatt del i denne sirkelen",
    "You have requested to join this circle" : "Du har forespurt deltagelse i denne sirkelen",
    "You have successfully left this circle" : "Du har forlatt denne sirkelen",
    "Cannot leave this circle" : "Kan ikke forlate denne sirkelen",
    "You have successfully deleted this circle" : "Du har slettet denne sirkelen",
    "Cannot delete this circle" : "Kan ikke slette denne sirkelen",
    " {type} '{name}' created" : " {type} '{name}' opprettet",
    " {type} '{name}' could not be created" : " {type} '{name}' kunne ikke opprettes",
    "Issue while retrieving the details of this circle" : "Feil ved innhenting av sirkeldetaljer for denne sirkelen",
    "Issue while retrieving the list of circles" : "Feil ved innhenting av sirkel-listen",
    "Group '{name}' successfully added to the circle" : "Gruppen '{name}' ble lagt til i sirkelen",
    "Group '{name}' could not be added to the circle" : "Gruppen '{name}' kunne ikke legges til i sirkelen",
    "Group '{name}' updated" : "Gruppen '{name}' ble oppdatert",
    "Group '{name}' could not be updated" : "Gruppen '{name}' kunne ikke oppdateres",
    "Group '{name}' successfully removed from the circle" : "Gruppen '{name}' ble fjernet fra sirkelen",
    "Group '{name}' could not be removed from the circle" : "Gruppen '{name}' kunne ikke fjernes fra sirkelen",
    "A link to <b>{remote}</b> could not be initiated" : "En lenke til <b>{remote}</b> kunne ikke opprettes",
    "A link to <b>{remote}</b> has been requested." : "En lenke til <b>{remote}</b> har blitt forespurt.",
    "the link to <b>{remote}</b> is now up and running." : "lenken til <b>{remote}</b> er nå oppe og går.",
    "The status of the link could not be updated" : "Lenkens status kunne ikke oppdateres",
    "The status of the link has been updated" : "Statusen for lenken har blitt oppdatert",
    "This operation will add/invite every members from the group to the circle" : "Dette vil legge til/invitere alle medlemmer i gruppen til sirkelen",
    "Member '{name}' successfully added to the circle" : "Medlemmet '{name}' lagt til i sirkelen",
    "Member '{name}' could not be added to the circle" : "Medlemmet '{name}' kunne ikke legges til i sirkelen",
    "Members from group '{name}' successfully added to the circle" : "Medlemmer av gruppen '{name}'  ble lagt til i sirkelen",
    "Members from group '{name}' could not be added to the circle" : "Medlemmer av gruppen '{name}'  kunne ikke legges til i sirkelen",
    "Member '{name}' successfully removed from the circle" : "Medlemmet '{name}' fjernet fra sirkelen",
    "Member '{name}' could not be removed from the circle" : "Medlemmet '{name}' kunne ikke fjernes fra sirkelen",
    "Member '{name}' updated" : "Medlemmet '{name}' oppdatert",
    "Member '{name}' could not be updated" : "Medlemmet '{name}' kunne ikke oppdateres",
    "Issue while saving settings" : "Problem under lagring av innstillinger",
    "Settings saved." : "Innstillinger lagret.",
    "Circles" : "Sirkler",
    "You created the circle {circle}" : "Du opprettet sirkelen {circle}",
    "{author} created the circle {circle}" : "{author} opprettet sirkelen {circle}",
    "You deleted {circle}" : "Du slettet {circle}",
    "{author} deleted {circle}" : "{author} slettet {circle}",
    "You joined {circle}" : "Du tok del i {circle}",
    "{member} joined {circle}" : "{member} tok del i {circle}",
    "You added {member} as member to {circle}" : "Du la til {member} som medlem i {circle}",
    "You were added as member to {circle} by {author}" : "Du ble lagt til som medlem i {circle} av {author}",
    "{member} was added as member to {circle} by {author}" : "{member} ble lagt til som et medlem i {circle} av {author}",
    "You left {circle}" : "Du forlot {circle}",
    "{member} left {circle}" : "{member} forlot {circle}",
    "You removed {member} from {circle}" : "Du fjernet {member} fra {circle}",
    "You were removed from {circle} by {author}" : "Du ble fjernet fra {circle} av {author}",
    "{member} was removed from {circle} by {author}" : "{member} ble fjernet fra {circle} av {author}",
    "You invited {member} into {circle}" : "Du inviterte {member} inn i {circle}",
    "You have been invited into {circle} by {author}" : "Du har blitt invitert inn i {circle} av {author}",
    "{member} have been invited into {circle} by {author}" : "{member} har blitt invitert inn i {circle} av {author}",
    "You changed {member}'s level in {circle} to %1$s" : "Du endret {member}s nivå i {circle} til %1$s",
    "{author} changed your level in {circle} to %1$s" : "{author} endret ditt nivå i {circle} til %1$s",
    "{author} changed {member}'s level in {circle} to %1$s" : "{author} endret {member}s nivå i {circle} til %1$s",
    "You requested an invitation to {circle}" : "Du forespurte en invitasjon til {circle}",
    "{member} has requested an invitation into {circle}" : "{member} har forespurt en invitasjon til {circle}",
    "You are the new owner of {circle}" : "Du er den nye eieren av {circle}",
    "{member} is the new owner of {circle}" : "{member} er den nye eieren av {circle}",
    "You sent a request to link {circle} with {link}" : "Du har forespurt lenking av {circle} med {link}",
    "{author} sent a request to link {circle} with {link}" : "{author} sendt en forespørsel om å lenke {circle} med {link}",
    "{link} requested a link with {circle}" : "{link} forespurte en lenke med {circle}",
    "The request to link {circle} with {link} has been rejected" : "Forespørselen om å lenke {circle} med {link} har blitt avslått",
    "The request to link {link} with {circle} has been canceled remotely" : "Den forespurte lenken {link} med {circle} har blitt avbrutt fra annensteds hen",
    "The request to link {circle} with {link} has been accepted" : "Forespørselen om å lenke {circle} med {link} har blitt godtatt",
    "You dismissed the request to link {link} with {circle}" : "Du avslo forespørselen om å lenke {link} med {circle}",
    "{author} dismissed the request to link {link} with {circle}" : "{author} avslo forespørselen om å lenke {link} med {circle}",
    "You canceled the request to link {circle} with {link}" : "Du avbrøt forespørselen om å lenke {circle} med {link}",
    "{author} canceled the request to link {circle} with {link}" : "{author} avbrøt forespørselen om å lenke {circle} med {link}",
    "You accepted the request to link {link} with {circle}" : "Du godtok forespørselen om å lenke {link} med {circle}",
    "{author} accepted the request to link {link} with {circle}" : "{author} godtok forepørselen om å lenke {link} med {circle}",
    "A link between {circle} and {link} is now up and running" : "En lenke mellom {circle} og {link} er nå oppe og går",
    "The link between {circle} and {link} has been shutdown remotely" : "Lenken mellom {circle} og {link} har blitt stengt ned fra annensteds hen",
    "You closed the link between {circle} and {link}" : "Du lukket lenken mellom {circle} og {link}",
    "{author} closed the link between {circle} and {link}" : "{author} lukket lenken mellom {circle} og {link}",
    "On events happening in a <strong>Circle</strong> you belong" : "På hendelser i en <strong>Sirkel</strong> du tilhører",
    "Any important event in a <strong>Circle</strong> you are moderating" : "Ønsker du å bli varslet om hendelser i <strong>sirkler</strong> du modererer?",
    "The name of your circle must contain at least 3 characters" : "Navnet på sirkelen din må inneholde minst 3 tegn",
    "The name of your circle cannot start with this character" : "Navnet på sirkelen kan ikke begynne med dette tegnet",
    "Federated circles are not allowed on this Nextcloud" : "Sammenknyttede sirkler tillates ikke i denne Nextcloud-en",
    "Linked Groups are not allowed on this Nextcloud" : "Lenkede grupper er ikke tillatt i denne Nextclouden",
    "You cannot use the Circles Application until your administrator has allowed at least one type of circles" : "Du kan ikke bruke sirkelprogrammet før administratoren din har tillat minst én type sirkler",
    "The circle does not exist or is hidden" : "Sirkelen finnes ikke eller er skjult",
    "A circle with that name exists" : "En sirkel med det navnet finnes",
    "Circle not found" : "Fant ikke sirkelen",
    "Federated Link not found" : "Fant ikke den sammenknyttede lenken",
    "This member does not exist" : "Dette medlemmet finnes ikke",
    "This user is already a member of the circle" : "Denne brukeren er allerede medlem av sirkelen",
    "This option is not available for personal circles" : "Dette valget er ikke tilgjengelig for personlige sirkler",
    "The circle is not Federated" : "Denne sirkelen er ikke sammenknyttet",
    "You cannot join this circle" : "Du kan ikke tilslutte deg denne sirkelen",
    "This member is not a moderator" : "Dette medlemmet er ikke en moderator",
    "Not enough privileges" : "Ikke nok privilegier",
    "This member is not the owner of the circle" : "Dette medlemmet eier ikke denne sirkelen",
    "This member is not admin of the circle" : "Dette medlemmet er ikke admin for sirkelen",
    "This member is the owner of the circle" : "Dette medlemmet eier denne sirkelen",
    "You are already a member of this circle" : "Du er allerede medlem av denne sirkelen",
    "You have been blocked from this circle" : "Du har blitt blokkert fra denne sirkelen",
    "You need a specify a type of circle" : "Du må oppgi sirkeltype",
    "You cannot create this type of circle" : "Du kan ikke opprette denne typen sirkel",
    "You cannot display this type of circle" : "Du kan ikke vise denne typen sirkel",
    "Federated link does not have a valid format" : "Sammenknyttet lenke ser ikke ut til å ha et gyldig format",
    "The remote host is down or the Circles app is not installed on it" : "Tjeneren er nede eller så er ikke Sirkelappen installert på den",
    "Federated circles are not allowed on the remote Nextcloud" : "Sammenknyttede sirkler tillates ikke på Nextcloud-en annensteds hen",
    "The remote circle does not accept Federated Links" : "Sirkelen annensteds hen tillater ikke sammenknyttede lenker",
    "It seems that you are trying to link a circle to itself" : "Det later til at du prøver å lenke en sirkel til seg selv",
    "This link exists already" : "Denne lenken finnes allerede",
    "The requested remote circle does not exist" : "Forespurt sirkel annensteds hen finnes ikke",
    "Cannot proceed with this status update" : "Kan ikke fortsette med denne statusoppdateringen",
    "This group does not exist" : "Denne gruppen finnes ikke",
    "This group is already linked to the circle" : "Gruppen er allerede lenket til sirkelen",
    "You cannot edit level in a personal circle" : "Du kan ikke endre nivå i en personlig sirkel",
    "Group cannot be set as owner of a circle" : "Gruppe kan ikke settes som eier av en sirkel",
    "This user does not exist" : "Denne brukeren finnes ikke",
    "Create a new circle" : "Opprett en ny sirkel",
    "Select a type of circle" : "Velg en type sirkel",
    "Create a personal circle" : "Opprett en personlig sirkel",
    "Create a hidden circle" : "Lag en skjult sirkel",
    "Create a private circle" : "Opprett en privat sirkel",
    "Create a public circle" : "Opprett en offentlig sirkel",
    "A personal circle is a list of users known only to the owner." : "En personlig sirkel er en liste over brukere bare eieren vet om.",
    "This is the right option if you want to do recurrent sharing with the same group." : "Dette er det rette valget hvis du vil utføre gjentagende deling med samme gruppe.",
    "An hidden circle is an open group that can be protected by a password." : "En skjult sirkel er en åpen gruppe som kan beskyttes med et passord.",
    "Users won't be able to find this circle using the Nextcloud search engine." : "Brukere vil ikke kunne finne denne sirkelen ved bruk av søkemotoren i Nextcloud.",
    "A private circle requires invitation or confirmation by an admin." : "En privat sirkel krever invitasjon eller bekreftelse av en administrator.",
    "This is the right circle if you are looking for privacy when sharing your files or ideas." : "Dette er den rette sirkelen hvis du søker privatlivets fred mens du deler dine filer eller ideer.",
    "A public circle is an open group visible to anyone willing to join." : "En offentlig sirkel er en åpen gruppe synlig for alle som ønsker å ta del i den.",
    "Everyone will be able to see and join your circle." : "Alle vil kunne se og ta del i din sirkel.",
    "personal circles" : "personlige sirkler",
    "hidden circles" : "skjult sirkel",
    "private circles" : "private sirkler",
    "public circles" : "offentlige sirkler",
    "All circles" : "Alle sirkler",
    "Search circles" : "Søk i sirkler",
    "No filter" : "Inget filter",
    "Circles you belong to" : "Sirkler du er en del av",
    "Circles you own" : "Sirkler du eier",
    "No circle selected" : "Ingen sirker er valgt",
    "Pending invitation to join this circle" : "Ventende invitasjon til å ta del i denne i sirkelen",
    "You have a pending request to join this circle" : "Du har en ventende forespørsel for deltagelse i denne sirkelen",
    "Add a member" : "Legg til en medlem",
    "Link a group" : "Lenk en gruppe",
    "Link a circle" : "Lenk sirkel",
    "Join this circle" : "Bli med i denne sirkelen",
    "Delete circle" : "Slett sirkel",
    "Edit circle" : "Endre sirkel",
    "Accept the invitation" : "Aksepter invitasjonen",
    "Decline the invitation" : "Avslå invitasjonen",
    "Leave this circle" : "Forlat denne sirkelen",
    "Link to a circle" : "Lenk til en sirkel",
    "Return to menu" : "Gå tilbake til meny",
    "Username" : "Brukernavn",
    "Level" : "Nivå",
    "Status" : "Status",
    "Joined" : "Ble med",
    "Group Name" : "Gruppenavn",
    "Link" : "Lenke",
    "Linked" : "Lenket",
    "Allow Federated Links" : "Tillat sammenknyttede lenker",
    "Turns the circle as Federated and enable sharing between Federated Circles" : "Knytter sammen en sirkel, og skrur på deling mellom sammenknyttede sirkler",
    "Automatically Accept Link Request" : "Godta lenkeforespørsel automatisk",
    "Warning: enabling this will automatically accept new Link requests from other circle." : "Advarsel: Å skru på dette vil medføre automatisk godkjenning av nye lenkeforespørsler fra den andre sirkelen.",
    "Save settings" : "Lagre innstillinger"
},
"nplurals=2; plural=(n != 1);");
